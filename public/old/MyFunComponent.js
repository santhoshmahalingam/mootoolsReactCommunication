(function() {
    ...
    var MyFunComponent = new Class({
        ...
        buttonClicked: function(event)  {
            ...
            const eventMootools = new CustomEvent('mootoolsEvent', {
                detail: {message : `Mootools says to React - ${text}`},
              });
              window.dispatchEvent(eventMootools);
        },
    ...
    window.addEvent('load', function() {
        ...
        window.addEventListener('reactEvent', event => {
            console.log(event.detail, funComponent);
            funComponent.setText(`${event?.detail?.message}`)
        });
    });
})();